...
### 在初始化（Initializer）方法和dealloc方法中不要使用访问器方法（Don’t Use Accessor Methods in Initializer Methods and dealloc）
...
## 使用弱引用避免循环保持（Use Weak References to Avoid Retain Cycles）
...
## 避免导致你正在使用的对象解除分配（Avoid Causing Deallocation of Objects You’re Using）
...
## 不要使用dealloc来管理稀缺资源（Don’t Use dealloc to Manage Scarce Resources）

通常你都不应该在dealloc方法里管理如文件描述符，网络连接，缓冲区或缓存这些稀缺资源。尤其，你不应该设计类，以便你认为它会被调用时调用dealloc。dealloc的调用可能由于错误或应用程序拆卸而延迟或回避。

相反，如果你有一个实例管理稀缺资源的类，你应该设计你的应用程序，以便在你不再需要这些资源的时候告诉该实例去“清理”。你通常会释放该实例，然后释放dealloc，但是如果没有，则你将不会再遇到其他问题。

如果你视图在dealloc上进行资源管理，可能会出现问题。例如：

1. 顺序依赖于对象图（[object graph]()）拆除  
   对象图拆除机制本质上是无序的。尽管你可能总是期望有一个特定的顺序，你正在引入脆弱性。例如，如果某个对象意外自动释放而不是释放，拆除顺序将会改变，这可能产生不可预期的结果。

2. 不开垦稀缺资源  
   内存泄露是需要修复的bug，但是他们通常不会立即致命。然而，如果稀缺资源没有在你期望的时候释放，你将会遇到更严重的问题。例如，如果你的应用程序用完了文件描述符，用户将无法保存数据。

3. 清理逻辑在错误的线程上执行  
   如果一个对象在意外的时间被自动释放，那么它将在它碰巧遇到的任何线程的自动释放池块中被释放。对于只应该从一个线程触及的资源，这可能很容易致命。

## 集合拥有它包含的对象（Collections Own the Objects They Contain）
