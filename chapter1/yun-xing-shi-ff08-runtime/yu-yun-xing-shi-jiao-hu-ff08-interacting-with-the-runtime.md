...
Cocoa中的大部分对象是NSObject类的子类，所以大部分对象继承了它定义的方法。（显著的异常是NSProxy类，更多信息请参阅[Message Forwarding](https://developer.apple.com/library/content/documentation/Cocoa/Conceptual/ObjCRuntimeGuide/Articles/ocrtForwarding.html#//apple_ref/doc/uid/TP40008048-CH105-SW1)。）因此它的方法建立的行为被固化到了每一个实例和每一个类对象。然而，少数情况下，NSObject类仅仅定义了事情如何处理的模板；它自己没有提供所有需要的代码。
