...
根据你对应用程序任务的理解，你应该已经能够辨别哪些代码将从并发中受益。如果改变任务中一个或多个步骤的顺序会改变结果，你很可能需要继续串行执行这些步骤。如果尽管改变了顺序也不会影响输出，你可以考虑并行执行这些任务。，定义将要执行的步骤等价的可执行工作单元。这些工作单元然后变成使用块或操作对象包括起来，并调度到合适的队列。
