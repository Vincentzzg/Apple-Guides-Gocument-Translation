...
对于你分辨出来的每一个可执行工作单元，至少在开始时不用担心将要执行的工作量太大。尽管运行一个线程总是一个开销，调度队列和操作队列的一大好处是在很多情况下这些开销比传统线程要小很多。因此，使用队列执行更小的工作单元比使用线程更高效是可能的。当然，你应该总是确保实际性能并按需调整任务的大小，但是开始时，任务不需要考虑的太小。
