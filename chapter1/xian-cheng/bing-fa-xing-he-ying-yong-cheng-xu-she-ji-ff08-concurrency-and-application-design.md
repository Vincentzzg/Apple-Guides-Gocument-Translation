...
所以，总结问题，应用程序需要一种方式以利用可变的计算机核心数量。单个应用程序执行的工作数量也需要能够动态地扩展以适应变化的系统条件。而且解决方案不得不足够简单以至于不会增加需要利用这些核心的工作数量。好消息是苹果的操作系统提供了所有这些问题的解决方案，这个章节将介绍构成这些解决方案的技术以及为了利用它们你可以对你的代码进行的设计调整。
